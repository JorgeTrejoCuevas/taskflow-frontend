...
export default function Login() {
  ...
  // ── Estado registro ──────────────────────────────────────
  const [reg, setReg] = useState({ nombre: '', apellido: '', correo: '', password: '', confirmar: '' })
 ...
 const handleRegistro = async (e) => {
  ...
  const errs = {
    ...
    password:  !validarPassword(reg.password) ? 'Mínimo 8 caracteres, letra, número y símbolo (@#$!%*?&)' : '',
    confirmar: reg.password !== reg.confirmar ? 'Las contraseñas no coinciden' : '',
  }
  setErroresReg(errs)
  ...
  try {
    ...
    login({ nombre: data.nombre, apellido: data.apellido, correo: data.correo, token: data.token })
  ...
  try {
    ...
    const data = await res.json()
    if (!res.ok) {
      setErroresLog({ password: data.error || 'Correo o contraseña incorrectos' })
      return
    }
    login({ nombre: data.nombre, apellido: data.apellido, correo: data.correo, token: data.token })
    navigate('/dashboard', { replace: true })
  ...
  const CampoPassword = ({ label, value, onChange, verPass, setVerPass, error, placeholder }) => (
    <div>
...
<span className="font-bold text-[#1C1A3A] text-lg">TaskFlow</span>
        ...
        <Link to="/" className="text-sm text-gray-500 hover:text-[#534AB7] transition-colors">Volver</Link>
      </header>
  ...
  />
            ...
            <h1 className="text-xl font-bold text-[#1C1A3A]">Accede a TaskFlow</h1>
            <p className="text-sm text-gray-500 mt-1">Crea tu cuenta o inicia sesión</p>
          </div>
          ...
          <div className="flex border-b border-gray-100">
            {['registro', 'login'].map(t => (
              <button
                key={t}
                onClick={() => { setTab(t); setErroresReg({}); setErroresLog({}) }}
                className={`flex-1 py-3 text-sm font-medium transition-colors border-b-2 ${
                  tab === t ? 'border-[#534AB7] text-[#534AB7]' : 'border-transparent text-gray-400 hover:text-gray-600'
                }`}
              >
              ...
              <form onSubmit={handleRegistro} className="space-y-4" noValidate>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" required placeholder="Carlos"
                      value={reg.nombre}
                    ...
                    />
                    {erroresReg.nombre && <p className="text-xs text-[#E24B4A] mt-1">{erroresReg.nombre}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">Apellido</label>
                    <input type="text" required placeholder="Ramírez"
                      value={reg.apellido}
                    ...
                    />
                    {erroresReg.apellido && <p className="text-xs text-[#E24B4A] mt-1">{erroresReg.apellido}</p>}
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Correo electrónico</label>
                  <input type="email" required placeholder="carlos@email.com"
                    value={reg.correo}
                  ...
                  />
                  {erroresReg.correo && <p className="text-xs text-[#E24B4A] mt-1">{erroresReg.correo}</p>}
                </div>

                <CampoPassword label="Contraseña" value={reg.password}
                  onChange={v => setReg({...reg, password: v})}
                  verPass={verPassReg} setVerPass={setVerPassReg}
                  error={erroresReg.password} placeholder="Mín. 8 chars, letra, número y símbolo"
                />

                <CampoPassword label="Confirmar contraseña" value={reg.confirmar}
                  onChange={v => setReg({...reg, confirmar: v})}
                  verPass={verConfirmar} setVerPass={setVerConfirmar}
                  error={erroresReg.confirmar} placeholder="Repite tu contraseña"
                />
                ...
                {reg.password && (
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-1">
                      {[reg.password.length >= 8, /[A-Za-z]/.test(reg.password), /\d/.test(reg.password), /[@#$!%*?&]/.test(reg.password)].map((ok, i) => (
                        <div key={i} className={`flex-1 h-1.5 rounded-full ${ok ? 'bg-[#1D9E75]' : 'bg-gray-200'}`} />
                      ))}
                    </div>
                    <div className="flex gap-1 text-xs text-gray-400">
                      <span className={reg.password.length >= 8 ? 'text-[#1D9E75]' : ''}>8+ chars</span>
                      <span>·</span>
                      <span className={/[A-Za-z]/.test(reg.password) ? 'text-[#1D9E75]' : ''}>letra</span>
                      <span>·</span>
                      <span className={/\d/.test(reg.password) ? 'text-[#1D9E75]' : ''}>número</span>
                      <span>·</span>
                      <span className={/[@#$!%*?&]/.test(reg.password) ? 'text-[#1D9E75]' : ''}>símbolo</span>
                    </div>
                  </div>
                )}

                <div className="flex items-center gap-2">
                  <input type="checkbox" required id="terminos" className="accent-[#534AB7]" />
                  <label htmlFor="terminos" className="text-xs text-gray-500">
                    Acepto los <span className="text-[#534AB7]">términos y condiciones</span>
                  </label>
                </div>

                <button type="submit" className="w-full bg-[#534AB7] text-white font-semibold py-3 rounded-xl hover:bg-[#3C3489] transition-colors text-sm">
                  Crear cuenta
                </button>
              </form>
            )}
            ...
            {tab === 'login' && (
              <form onSubmit={handleLogin} className="space-y-4" noValidate>
                <div>
                  ...
                  />
                  {erroresLog.correo && <p className="text-xs text-[#E24B4A] mt-1">{erroresLog.correo}</p>}
                </div>

                <CampoPassword label="Contraseña" value={log.password}
                  onChange={v => setLog({...log, password: v})}
                  verPass={verPassLog} setVerPass={setVerPassLog}
                  error={erroresLog.password} placeholder="Tu contraseña"
                />
                ...
                <p className="text-center text-xs text-gray-500 mt-2">
                  ¿No tienes cuenta?{' '}
                  <button type="button" onClick={() => setTab('registro')} className="text-[#534AB7] font-medium hover:underline">
                    Regístrate gratis
                  </button>
                </p>
              </form>
