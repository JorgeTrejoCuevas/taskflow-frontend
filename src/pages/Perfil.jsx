  ...
  const cambiarPassword = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/cambiar-password`, {
        ...
        body: JSON.stringify({ passwordActual: pass.actual, passwordNueva: pass.nueva })
      })
      if (res.status === 401) {
      ...
      if (!res.ok) throw new Error()
      setExito('Contraseña actualizada. Por seguridad inicia sesión nuevamente.')
    ...
    <div className="min-h-screen bg-[#F8F7FF]">
      ...
      <header className="bg-white border-b border-gray-100 px-6 h-16 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center overflow-hidden p-1 shadow-md border border-gray-100">
            <img src="/taskflow-logo.png" alt="TaskFlow" className="w-full h-full object-contain" />
          </div>
          <span className="font-bold text-[#1C1A3A] text-lg">TaskFlow</span>
        </div>
        <div className="flex items-center gap-3">
          <Link to="/dashboard" className="text-sm text-[#534AB7] hover:underline font-medium">
            ← Volver al dashboard
          </Link>
          <button onClick={cerrarSesion} className="text-sm text-[#E24B4A] border border-[#F09595] hover:border-[#E24B4A] px-3 py-1.5 rounded-lg font-medium transition-colors">
            Cerrar sesión
          </button>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10 space-y-6">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-semibold text-gray-800 flex items-center gap-2">
              <i className="fa-solid fa-user text-[#534AB7]"></i> Datos personales
            </h2>
            {!editando && (
              <button onClick={() => setEditando(true)} className="text-sm text-[#534AB7] hover:underline font-medium">
              ...
              <form onSubmit={guardarDatos} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" required value={datos.nombre}
                  ...
                  <div>
                    ...
                    <input type="text" required value={datos.apellido}
                ...
                <div>
                  ...
                  <input type="email" disabled value={usuario?.correo}
        ...
        <div className="bg-white border border-[#F09595] rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-5">
            <p className="text-sm text-gray-500 mb-4">Al cerrar sesión necesitarás iniciar sesión nuevamente para acceder al dashboard.</p>
